def caminos(G, salida):
    lista_caminos_completos = []
    copia_grafo = copia(G)
    # copia el grafo para poder trabajar con él
    guarda_caminos = posiblesinicios(copia_grafo, salida)
    # mete en guarda_caminos los posibles inicios
    copia_grafo=quita_inicios(guarda_caminos, copia_grafo)
    # quita del grafo copiado los inicios posibles
    while len(guarda_caminos) > 0:  # mientras guarda_caminos siga conteniendo algo...
        camino = guarda_caminos.pop()  # saca un posible inicio y crea un camino con ese #TUNEAR ESTO SI ES POSIBLE
        for i in sigue_camino(camino[len(camino) - 1], copia_grafo):
            # recorre nodos conectados entre sí para cada nodo, de forma que para una lista (en verdad sería una tupla) del grafo, si no es el nodo final...
            if 0 in i:
                camino_nuevo = camino + [i]
                lista_caminos_completos.append(camino_nuevo)
                # de no ser así, ese nodo lo añade a camino nuevo y ese camino lo añade a la lista de caminos. por eso pone else, puesto que un camino se cierra si en el último nodo tenemos un cero o no podemos continuar por ese camino.
            else:  # lo que ocurre es que cada vez que hace un guarda_caminos.pop() la mayoría de las veces entra en este if, en el primer pop saca un inicio y lo junta en una lista con el siguiente nodo, y lo vuelve a guardar en guarda_caminos, de forma que luego hace otra vez el pop pero esta vez del inicio y el siguiente nodo y así sucesivamente hasta que en el último nodo hay un cero, una vez tenemos esto sacamos el camino completo de guarda_caminos; cuando estén todos los caminos completados significa que guarda_caminos está vacía y por ello el bucle while se cierra.
                camino_nuevo = camino + [i]
                guarda_caminos.append(camino_nuevo)
                copia_grafo.remove(i)  # añade el nodo en cuestión al inicial (o aquellos que lo preceden) y luego todo a guarda_caminos y lo quita de la copia del grafo
    return lista_caminos_completos


def copia(G):
    return [i for i in G]

def posiblesinicios(copia_grafo, salida):
    return   [[i] for i in copia_grafo if salida in i]

def sigue_camino(nudo, L):
    prox = []  # esta función es simple, lo que hace es buscar el siguiente nodo al dado
    for i in L:
        if nudo[0] in i or nudo[1] in i:
            prox.append(i)
    return prox
def quita_inicios(guarda_caminos, copia_grafo):
    for i in guarda_caminos:
        for k in i:
            copia_grafo.remove(k)
    return copia_grafo

def maze_solver(G, salida):
    salida_del_laberinto = []
    longitudes = []
    if len(caminos(G, salida)) == 0:  # caso particular ****puedes probar con if !caminos(G, salida):
        return ()
    # LO DE ABAJO ES UNA FORMA MÍA DE ESCOGER EL CAMINO MÁS CORTO. MI FORMA ES MEJOR PORQUE VALE PARA CUALQUIER CASO; ADEMÁS NO ME PILLO LOS DEDOS YA QUE TODO EL CÓDIGO QUE LE HE BORRADO A PACO PODRÍA SER ÚTIL (LAS GALLINAS QUE ENTRAN POR LAS QUE SALEN)
    if G:
        if salida in G[0]:
            if 0 in G[0]:
                return [list(G[0])]
    ...
    camino_corto = K[longitudes.index(min(longitudes))]
    if len(G) == 1:
        if camino_corto[0] != salida:
            return tuple(camino_corto.reverse())
        else:
            return (tuple(camino_corto),)  # caso particular, en concreto el segundo caso de las pruebas
    for i in camino_corto:  # ahora convierte el resultado en una tupla de tuplas y lo devuelve
        salida_del_laberinto.append(tuple(i))
    '''
    ****
    salida_del_laberinto = [tuple(i) for i in camino_corto] 
    '''
    return tuple(salida_del_laberinto)


