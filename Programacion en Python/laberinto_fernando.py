...
def recorre(G, I, camino, partida, soluciones=[], salida=[]):
    if len(G) != 0:
        if partida!=[]:
            ...
            conexion = conect(G,I)
        if conexion != -1 and conexion in G:
            if conexion[0]==I:
                I = conexion[1]
            else:
                I=conexion[0]
...
def maze_solver(G,origen):
    ...
    salidas=todas_salidas(copia,origen)
    for i in salidas:
        for j in i:
            if 0 in j:
                salida = i
    if origen not in salida[0]:
        salida = recodificar(salidas, salida[0],salida)
    lista = cortaencero(salida)
