def siguiente(tupla, origen):
    for i in tupla:
        if i !=origen:
            return i

def conecta(G, origen):
    lista=[]
    for i in G:
        if origen in i:
            elemento = siguiente(i, origen)
...
def camino(G,origen):
    ...
    vueltas=0
    copiaG = [i for i in G]
    while len(copiaG) > 1 and i <= len(copiaG):
        if vueltas == len(copiaG):
            return nodos
        if i == len(copiaG):
            i=0
            vueltas+=1
        if origen in copiaG[i]:
            nodos.append(copiaG[i])
            origen = siguiente(copiaG[i], origen)
            copiaG.pop(i)
        i += 1
    return nodos

def caminoconsalida(G, recorrido):
    for j in range(len(recorrido)):
        if 0 in recorrido[j]:
            print(tuple(recorrido[j+1]))
            return True

def todoscaminos(G,origen, pasados):
    pasados.append(origen)
    lista=conecta(G,origen)
    recorrido = camino(G, origen)
    print(lista)
    print(recorrido)
    if caminoconsalida(G, recorrido):
        return "Salida"
    for elemento in lista:
        if elemento not in pasados:
            todoscaminos(G,elemento, pasados)
    return -1
