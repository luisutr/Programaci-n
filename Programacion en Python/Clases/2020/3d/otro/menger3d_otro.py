...
class Menger3D:
    ...
    def divide(self, bbox, depth):
        ...
        w = math.sin(float(x1 - x0) / 5)
        h = math.sin(float(y1 - y0) / 5)
        d = math.sin(float(z1 - z0) / 5)
    ...
    def delete(self, cubes):
        for n in range(len(self.holeLUT)):
            ...
            if index < len(cubes):
                ...
                self.deletedCubes.append(deleted)
        return cubes
...
if __name__ == "__main__":
    bounds = [-1, 0, -1, 1, 2, 1]
    removals = [24, 18, 16, 14, 10, 8, 2]
    menger3d = Menger3D(bounds, 4, removals)
    menger3d.writeAsCubes('./menger3d.rib')
    menger3d.writeAsCubes('./menger3d_holes.rib', '')
