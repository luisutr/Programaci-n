...
def comunidades_autonomas(directorio):
    ...
    for i in range(len(archivo)):
        if (i > 10 and i < 29):
            fila = archivo[i].strip()  # elimina espacios en blanco de delante y detras de la linea(strip)
            celdas = fila.split(',')  # separa los caracteres en este caso por una coma (split)
            for j in range(len(celdas)):
                if j == 0:
                    lista_comunidades.append(celdas[j])
    archivo.close()
    return lista_comunidades


print comunidades_autonomas(ruta)
