...
c = conn.cursor()
c.execute(
    "CREATE TABLE IF NOT EXISTS `entries` (`id`	TEXT UNIQUE, `url`	TEXT, `userid`	TEXT, `username` TEXT, `taken_at` INTEGER, `filename` TEXT);")
if not prefs["quiet"]: print("Descargando las historias! :D")

while len(prefs["ids"]):
    pair = prefs["ids"].popitem()
    response = requests.request("GET", "https://i.instagram.com/api/v1/feed/user/" + pair[0] + "/reel_media/",
    ...
    for item in response['items']:
        ...
        try:
            c.execute('INSERT INTO entries VALUES(?,?,?,?,?,?)',
                      (str(id), str(url), str(userid), str(username), taken_at, ""))
