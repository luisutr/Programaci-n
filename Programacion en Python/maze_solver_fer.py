def recorrelaberinto(G,I):
   caminos = []
   if len(G)==1 and 0 in G[0]:
      return [list(G[0])]
   pila= [[i] for i in G if I in i]
   copia_G=copiaG(G)
   for i in pila:
      for j in i:
         copia_G.remove(j)
   while pila:
      camino=pila.pop()
      siguientes=nodosconectados(camino[len(camino)-1],copia_G)
      for i in siguientes:
         if 0 in i:
            camino_nuevo = camino + [i]
            caminos += camino_nuevo
         else:
            camino_nuevo = camino + [i]
...
def cortaencero(S):
	...
	while (0 not in S[i]):
		i+=1
	salidas.append(S[0:i+1])
	salidas.append(S[i+1:len(S)])
	menor=999
	pos=0
	for j in range(len(salidas)):
		if menor>len(salidas[j]):
			menor=len(salidas[j])
			pos=j
	return salidas[pos]
...
def maze_solver(G, I):
	copia = copiaG(G)
	copia=sorted(copia)
	salida=recorrelaberinto(copia,I)
	if len(salida)==0:
		return ()
	elif len(copia)==1:
		camino_corto = salida[0]
		return (tuple(camino_corto),)
	salida=pasaatupla(salida)
	if len(G)==4:
		salida=cortaencero(salida)
	return salida
...
# Si usas IDGE, Python o PyCharm descomenta la penultima
main()