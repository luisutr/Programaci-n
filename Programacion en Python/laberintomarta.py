...
def caminosalt(G,ver):
    veces=0
    for i in G:
        if ver[0] in i or ver[1] in i:
            veces+=1
            if veces>0:
                return i
    return -1

def saledellab(sol,salida):
    for i in sol:
        if salida in i:
            return True
    return False

def rebuild(salida,sol):
    solucion=[]
    punto=sol[0][0]
    for i in range(len(salida)):
        if punto in salida[i]:
            solucion = salida[0:i]+sol
    return solucion
...
def maze_solver(G,k):
    ...
    G = sorted(G)
    copiadeg=copiaG(G)
    lista = verticesadj(G, k)
    for i in lista:
        sol = laberinto(copiadeg,k,[])
        if G.index(i) in copiadeg:
            copiadeg.pop(G.index(i))
        #print(sol)
        if len(sol)!=0:
            salidas.append(sol)
        if saledellab(sol,0):
            return pasaatupla(sol)
    # recorro para ver si hay por sus nodos algun camino alternativo
    for salida in salidas:
        for j in salida:
            alter=caminosalt(copiadeg, j)
            if alter != -1:
                lista = verticesadj(copiadeg, alter[1])
                for i in lista:
                    sol = laberinto(copiadeg, alter[1], [])
                    sol.insert(0,alter)
                    if G.index(i) in copiadeg:
                        copiadeg.pop(G.index(i))
                    #print(sol)
