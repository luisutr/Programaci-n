# RESOLVEMOS EL CA INO MAS CORTO USANDO SALIR DEL LABERINTO
#HAY QUE MODIFICARLO, PARA QUE LA SALIDA SEA EL VERTICE QUE DIGAN Y NO 0
    ...
    if G !=[]:
        for i in range(len(G)):
            if nodo in G[i]:
                ...
                return G,v
    return G, v
def recorrelaberinto( G,origen,sol):
    G, vertady = verticeadj(G,origen)
    sol.append(vertady)
    if 0 in vertady:
        return sol
    origen = signodo(origen,vertady)
    if vertady != []:
        recorrelaberinto(G,origen,sol)
    return sol
def copiagrafo(G):
    copia=[]
    for i in G:
        copia.append(i)
    return copia
def salirdellaberinto(G,origen,sol):
    if len(G) > 0:
        adyacentes=listavertices(G,origen)
        for i in adyacentes:
            copia = copiagrafo(G)
            camino = recorrelaberinto(copia, origen, [])
            G.remove(i)
            ...
            if 0 in camino[-1]:
                sol.append(camino)
            salirdellaberinto(G,signodo(origen,i),sol)
    return sol
def maze_solver(G, origen):
    G = tuplas2listas(G)
    salida = salirdellaberinto(G,origen,[])
    salidacorta = []
    for i in salida:
        if 0 not in i:
            salidacorta.append(tuple(i))
        else:
            return tuple(salidacorta)
